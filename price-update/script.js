...
/*************************************************/
const GAS_WEB_APP_URL = "https://script.google.com/macros/s/AKfycbyuKYDv_jc4_FeVri0ck46MLt3r9JpaNh9pu6ooduierF5FU935AuieRSRZvdF7ja7E/exec";
let database = [];
let entries = [];
...
// MODIFIED: Globals for new branch logic
let selectedBranch = '';
let allBranches = []; // Will be populated from the API
...
/*************************************************/
// NEW: Fetches branches from the API
async function fetchBranches() {
    try {
        const response = await fetch(`${GAS_WEB_APP_URL}?action=getBranches`);
        if (!response.ok) throw new Error(`Network error: ${response.statusText}`);
        const result = await response.json();
        if (result.success) {
        ...
        displayMessage(`Error loading branches: ${error.message}`, true);
        console.error(error);
    }
}

// MODIFIED: Populates modal from the fetched `allBranches` array
function openBranchSelectionModal() {
    if (allBranches.length === 0) {
        displayMessage("Branch list is empty or could not be loaded.", true);
        return;
    }
    ...
    allBranches.forEach(branch => {
        ...
        const cell = row.insertCell();
        cell.textContent = branch.name;
        cell.style.cursor = 'pointer';
        cell.style.padding = '8px 12px';
        cell.onmouseover = function() { this.style.backgroundColor = '#f0f0f0'; };
        cell.onmouseout = function() { this.style.backgroundColor = 'white'; };
        cell.onclick = function() { selectBranch(branch.name); };
    });
    $('#branchSelectionModal').modal({ backdrop: 'static', keyboard: false });
    $('#branchSelectionModal').modal('show');
}

// UNCHANGED: This function works as is
...
// NEW: Main logic to handle automatic branch selection
async function initializeBranchLogic() {
    await fetchBranches();

    // The auth script in index.html makes `currentUserData` globally available
    if (typeof currentUserData === 'undefined' || !currentUserData) {
        console.error("User data not found. Cannot determine branch.");
        openBranchSelectionModal(); // Fallback to manual selection
        return;
    }
    ...
    if (userBranchCode) {
        const assignedBranch = allBranches.find(b => 
            String(b.code).trim() === String(userBranchCode).trim()
        );

        if (assignedBranch) {
            console.log("Found matching branch, selecting automatically:", assignedBranch.name);
            selectBranch(assignedBranch.name);
        } else {
            console.warn(`Assigned branch code '${userBranchCode}' not found in the branches list. Opening manual selector.`);
            openBranchSelectionModal();
        }
    } else {
        console.log("No assigned branch code found for user. Opening manual selector.");
        // If user has no specific branch (e.g., an admin), let them choose.
        openBranchSelectionModal();
    }
}
...
$(document).ready(function() {
    // Make sure auth check is complete before running main logic
    if (currentUserData === null) {
        // This case can happen if the auth script fails catastrophically, but is unlikely.
        // The redirect in the auth script should handle non-logged-in users.
        console.error("Authentication failed or is pending, halting app initialization.");
        return;
    }

    // MODIFIED: New initialization flow
    initializeBranchLogic(); // This handles both automatic and manual branch selection
