document.addEventListener('DOMContentLoaded', () => {
    ...
    const handleBranchSelect = async () => {
        ...
        currentBranch = { BranchCode: selectedOption.value, BranchName: selectedOption.dataset.branchName };
    ...
    const handleFormSubmit = async (e) => {
        ...
        if (success) {
            ...
            handleCancel();
            if (reportView.classList.contains('active-view')) { await fetchData('getLogs').then(data => { logs = data; renderLogs(); }); } else { logs = []; }
        }
    };
    ...
    // --- INITIALIZATION ---
    const init = async () => {
        if (GAS_URL === 'YOUR_GOOGLE_APPS_SCRIPT_URL_HERE') { alert('Please set your Google Apps Script URL in script.js'); return; }
        ...
        hideLoader();
        branchConfirmBtn.addEventListener('click', handleBranchSelect);
